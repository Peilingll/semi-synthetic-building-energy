...
However, both routes achieved a recall of approximately 0.11 for classes D--G, so the small difference between them does not imply strong binary classification.
...
The reported agreement statistics describe the adopted aggregation rules rather than strict upper bounds on achievable prediction performance.
...
The reported model performance is also bounded by the predefined scope of model development.
...
In addition, the $H'_{tr}$ analysis compares archetype assignments obtained from \ac{SVI} and reference data.
It does not validate either assignment against measured heat loss or energy consumption.

The geographical evidence is limited to four Dutch cities and supports only a bounded assessment of transfer.
...
An open source \ac{VLM} fine tuned on the experimental data should be compared with frozen DINOv2 and configurations in which the \ac{CNN} is fully fine tuned.
