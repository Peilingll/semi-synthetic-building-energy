...
\begin{table}[htbp]
	\centering
	\caption[Per-class classification report for the binary task.]{Per-class classification report for the binary task (\cref{tab:binary_routes}; hold-out set, $n = 2{,}018$; A--C support = 1,416; D--G support = 602).}
...
M1 and the trained M3 routes show high A--C recall (0.927--0.960) but low D--G recall (0.098--0.110).
M2--DINOv2 provides the most balanced class-level result, with D--G recall of 0.469 and the highest A--C precision.
M2--InternVL3 assigns every building to D--G; its D--G F1 of 0.459 therefore reflects the class prevalence (precision = 0.298) rather than discrimination between buildings.
