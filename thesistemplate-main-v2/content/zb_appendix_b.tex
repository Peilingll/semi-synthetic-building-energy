...
This appendix reports the complete training configurations for the two supervised vision models in \cref{sec:vision_inference_strategies} and the LightGBM classifiers in \cref{sec:downstream_model}.
...
A single graphics processing unit (GPU) --- an NVIDIA GeForce RTX 4060 Laptop GPU with 8\,GB of video memory (VRAM) --- was used for all model training and \ac{VLM} inference.
...
Validation and hold-out inference use deterministic resizing to a short side of 256 pixels followed by a 224-pixel centre crop.
