...
The attribute \acs{MLP} in each supervised model uses Gaussian error linear unit (GELU) activation.
...
The direct energy class models follow the same training procedure as \cref{tab:vision_hyperparams}, except that the three attribute heads are replaced by one energy class classification head and early stopping uses validation energy class macro F1.
The direct ResNet-50 model is fine-tuned end to end using the learning rates and batch size reported above.
The direct DINOv2 model first extracts and caches building level mean features because its backbone parameters are fixed. Image processing uses deterministic resizing and centre cropping without augmentation.
...
Unlike the supervised attribute prediction procedure in \cref{tab:vision_hyperparams}, this cached feature procedure does not use image augmentation during training.
...
\begin{table}[htbp]
	\centering
	\caption{Fixed LightGBM hyperparameters used for both energy class tasks and all attribute input conditions.}
...
The same hyperparameters are used for all attribute based conditions and both energy class tasks.
Only the objective changes with the task; no separate tuning is performed for individual input conditions.
