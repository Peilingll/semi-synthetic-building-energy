...
The main text reports only the corrected system and results, while this appendix documents the revisions needed to reproduce the analysis and reconcile differences with earlier project stage documents, including the interim presentation.
...
\begin{longtblr}[
	...
	Count reconciliation          & Building counts reported at different stages referred to different datasets.                                                                                                                               & Standardised the convention of reporting 10,104 buildings in the filtered \ac{SVI} dataset and 10,086 buildings in the experimental dataset.                                                                                                                      \\
	Certificate chronology        & The database contains only current certificates, and only 0.1\% of dwellings nationwide have a certificate time series.                                                                                      & The available data did not support longitudinal renovation labels; renovation detection was deferred to future evaluation in \cref{sec:validity_limitations}.                                                                                                     \\
...
\begin{table}[htbp]
	...
	\begin{tblr}{
		...
		\texttt{shape\_factor}          & \texttt{envelope\_area / volume}                                              & A comparison variant that includes shared walls is also documented.        \\
		\bottomrule
	\end{tblr}
	\par\vspace{0.4em}
...
\begin{table}[htbp]
	\centering
	\caption[Per-class classification report for the binary task.]{Per-class classification report for the binary task (\cref{tab:binary_conditions}; comparison subset, $n = 2{,}016$; support for classes A to C = 1,414; support for classes D to G = 602).}
	...
	\begin{tblr}{
		...
		\toprule
		Information source and configuration & A-C P        & A-C R        & A-C F1       & D-G P        & D-G R        & D-G F1       \\
		\midrule
		Uniform random          & 0.702          & 0.500          & 0.584          & 0.298          & 0.500          & 0.373          \\
		Reference attributes    & 0.715          & \textbf{0.960} & \textbf{0.819} & \textbf{0.509} & 0.098          & 0.165          \\
		Direct image, DINOv2    & \textbf{0.764} & 0.731          & 0.748          & 0.426          & 0.469          & \textbf{0.447} \\
		Direct image, ResNet-50 & 0.741          & 0.741          & 0.741          & 0.390          & 0.391          & 0.391          \\
		Direct image, InternVL3 & 0.000          & 0.000          & 0.000          & 0.298          & \textbf{1.000} & 0.459          \\
		Predicted attributes, DINOv2    & 0.712          & 0.936          & 0.809          & 0.419          & 0.108          & 0.172          \\
		Predicted attributes, ResNet-50 & 0.710          & 0.927          & 0.804          & 0.391          & 0.110          & 0.171          \\
		Predicted attributes, InternVL3 & 0.719          & 0.698          & 0.708          & 0.335          & 0.359          & 0.347          \\
		\bottomrule
	\end{tblr}
\end{table}

The macro values in \cref{tab:binary_conditions} are the unweighted means of the two corresponding class-level values reported here.
The reference attribute condition and the trained predicted attribute conditions show high recall for classes A to C (0.927 to 0.960) but low recall for classes D to G (0.098 to 0.110).
Direct image prediction with DINOv2 provides the most balanced class-level result, with a recall of 0.469 for classes D to G and the highest precision for classes A to C.
Direct image prediction with InternVL3 assigns every building to classes D to G; its F1 score of 0.459 for these classes therefore reflects the class prevalence (precision = 0.298) rather than discrimination between buildings.
The uniform random condition reports the analytical expectation, with precision equal to each class prevalence and recall equal to 0.5.
