...
This sequence separates model prediction from deterministic archetype assignment and uses the BAG building as the unit of comparison throughout.
...
The reference branch produces one record per BAG building that passes the reference filters, while the image branch produces one record per selected image.
The branches are linked by the BAG building identifier \texttt{pand\_id}, so one building reference record may correspond to several street view image records.
...
Dataset construction uses the registered EP-Online class from the most recent linked certificate.
This class may describe one dwelling or the condition recorded on one registration date rather than the complete BAG building.
...
This study adds a BAG residential filter to the image acquisition workflow.
...
The centroid of each candidate footprint is intersected with BAG polygons, and only footprints linked to a residential BAG building proceed to image acquisition.
Each accepted crop is linked to the corresponding BAG building through \texttt{pand\_id}, which joins the image record to the eligible reference set.
