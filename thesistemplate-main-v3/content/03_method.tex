...
It connects building attributes, reconstructed geometry, registered energy classes, and images, and the integrated dataset records the source of each field.
...
Throughout the four stages, the integrated dataset records the provenance of three field groups: reference data, model outputs, and archetype properties.
...
The administrative fields come from registry records. The 3DBAG fields describe geometry reconstructed from BAG and national elevation data.
...
Stage 1 constructs the linked dataset required for all subsequent operations.
The reference data branch produces one record per eligible BAG building, while the image branch produces one record per selected image.
...
\begin{figure}[htbp]
	...
	\caption[Simplified Stage 1 record structure.]{Simplified Stage 1 record structure. Each \texttt{BuildingReferenceRecord} represents one eligible BAG building and is linked through \texttt{pand\_id} to one to eight \texttt{StreetViewImageRecord} entries. Each image record represents one selected facade crop.}
...
After the records are ordered by registration date, repeated entries associated with the same recorded BAG identifier field are reduced to the most recent entry before linkage.
Entries that list several BAG identifiers are then expanded to produce one linked record for each \texttt{pand\_id}.
...
The registration date is included to assess whether a record falls within the period governed by NTA 8800 \parencite{NTA8800}, but a date alone does not verify the calculation method used for an individual certificate.
The original EP-Online class is stored without modification during dataset construction.
...
This study adds a BAG residential filter and stores the BAG building identifier with every accepted crop, allowing the image records to be joined to the reference table.
...
This limitation narrows the scope of the subsequent evaluation, but it does not change the image selection procedure.
...
The resulting mapping stores both the source footprint identifier and \texttt{pand\_id}, so the BAG identifier remains available after detection, reprojection, and cropping.

For each accepted footprint, Mapillary panorama metadata are queried within the study area.
Candidate views are screened by camera distance, unobstructed visibility to the target building, and \acf{AoV}.
The eligible views are ranked and limited at footprint level according to the criteria in Table~\ref{tab:data_criteria} and downloaded at 2,048 pixels.
...
\begin{figure}[htbp]
	...
	\caption[Image transformation from a 360\textdegree{} panorama to facade crops.]{Image transformation from a selected 360\textdegree{} panorama to facade crops for one building in Rotterdam. The sequence shows the raw panorama, candidate view selection, Grounding DINO detection, perspective reprojection, and the crops selected for the BAG building. Candidate crops are ranked by \ac{AoV}. Imagery \textcopyright{} Mapillary contributors (CC BY-SA 4.0).}
...
The linked dataset is restricted to buildings with at least one eligible image, processable 3DBAG geometry, and attributes that permit TABULA-NL assignment.
...
This temporal difference is reported as a limitation because the available records do not support verified date matching.
...
\begin{table}[htbp]
	...
	\begin{tblr}{
		...
		Reference & Record linkage & BAG, EP-Online, and 3DBAG records linked by normalised \texttt{pand\_id}, with one reference record selected per building \\
		...
		Integration & Linked dataset & Reference record and at least one eligible image available, processable 3DBAG geometry, and valid TABULA-NL input attributes \\
		\bottomrule
	\end{tblr}
\end{table}
...
A two-step partitioning procedure separates model development from final evaluation.
...
Using the same partitions for building attribute prediction, archetype assignment, and energy class prediction prevents differences between methods from reflecting different building samples.
...
The Stage 2 output records the model configuration and predicted attributes for each building.
...
Both supervised model configurations are fitted separately in each development fold.
Validation macro F1 for building type controls early stopping and selects one checkpoint within each fold.
...
The subset is stratified by reference building type, with minority types deliberately oversampled to prevent prompt assessment from being dominated by the most frequent building type.
...
Both procedures use the same building partitions and target definitions, which allows their predictions to be compared without changing the evaluation sample.
...
A separate seven-class task combines these ratings with class A and evaluates prediction at the finer resolution of classes A to G.
...
ResNet-50 and DINOv2 use the same image processing, feature aggregation, and parameter update settings as attribute prediction, but replace the three attribute heads with one energy class head.
