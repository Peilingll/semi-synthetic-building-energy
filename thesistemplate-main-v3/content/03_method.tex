...
This chapter defines a four-stage method for predicting building attributes from street view imagery and evaluating their effects on archetype assignment and registered energy class prediction.
One BAG building is the unit of analysis and linkage throughout the method, and its identifier is stored as \texttt{pand\_id}.
Building attributes refer to building type, construction year, and floor count.
Reference building attributes are obtained from EP-Online, BAG, and reconstructed 3DBAG data, while predicted building attributes are produced by the vision configurations.
...
Stage 1 constructs the experimental dataset from reference records and eligible street view images.
...
This sequence separates model prediction from deterministic archetype assignment and preserves the BAG building as the unit of comparison.
...
Source data comprise administrative \acf{BAG} and EP-Online records and reconstructed 3DBAG geometry \parencite{kadasterbag,rvoeponline,threedbagdocs,peters2022,dukai2021}.
...
The reference branch produces one record per eligible BAG building, while the image branch produces one record per selected image.
...
\begin{table}[htbp]
	...
	\begin{tblr}{
		...
		\midrule
		BAG & Building identifier, construction year, two-dimensional footprint, status, and use & Linkage key, reference construction year, and residential building filter \\
...
Records satisfying these criteria form the eligible reference set, with one record per building containing the reference building attributes, reconstructed geometry, and original registered energy class.

Certificates are not filtered by calculation method.
The registration date is included to assess whether a record falls within the period governed by NTA 8800, the Dutch method for determining building energy performance \parencite{NTA8800}, but a date alone does not verify the calculation method used for an individual certificate.
...
This study adds a BAG residential filter and stores the BAG building identifier with every accepted crop, allowing the image records to be joined to the eligible reference set.
...
Final integration joins the eligible reference set to the \ac{SVI} manifest by \texttt{pand\_id}.
The linked data product used in the experiments is termed the experimental dataset.
It is restricted to buildings with at least one eligible image, processable 3DBAG geometry, and reference building attributes that permit TABULA-NL assignment.
...
\begin{table}[htbp]
	...
	\begin{tblr}{
		...
		Integration & Experimental dataset & Reference record and at least one eligible image available, processable 3DBAG geometry, and valid TABULA-NL input attributes \\
		\bottomrule
	\end{tblr}
\end{table}
...
Partitioning is performed on the experimental dataset before model fitting.
...
Stage 2 predicts building type, construction year, and floor count from the street view images linked to each BAG building.
...
For assignment from reference building attributes, the BAG construction year supplies $y_i$ and the EP-Online building type is converted to $s_i$ using Table~\ref{tab:type_mapping}.
...
Comparisons among energy class conditions are restricted to holdout buildings with valid outputs for every condition; this intersection is termed the comparison subset.
