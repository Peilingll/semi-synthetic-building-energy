...
The question was investigated through a Dutch case in which building type, construction year, and floor count inferred from \ac{SVI} were compared with administrative reference attributes, used for TABULA-NL archetype assignment, and evaluated as inputs to registered energy class prediction.
...
The three vision configurations produced estimates for all three attributes, but prediction quality differed by attribute, class, and model configuration.
...
DINOv2 and ResNet-50 achieved joint cell accuracies of 0.825 and 0.826, respectively, but the majority cell baseline already reached 0.790.
Their macro cell recalls of 0.180 and 0.128 show that the high overall agreement was concentrated in frequent combinations of building type and construction period rather than distributed across the occupied archetype cells.
The errors also changed the assigned thermal parameters.
Relative to calculations based on reference cells, the cells assigned from DINOv2 and ResNet-50 predictions produced aggregate transmission heat transfer coefficients that were 10.0\% and 10.5\% higher, while InternVL3-2B produced a value 16.6\% higher.
...
Changing from reference attributes to DINOv2 predicted attributes reduced binary macro F1 by only 0.002, but reduced seven class macro F1 by 0.022.
...
Among the structured inputs, construction year made the largest marginal contribution to energy class prediction and should therefore be prioritised when improving visual attribute extraction.
...
When administrative attributes are incomplete, predictions of building type, construction year, and floor count can support preliminary building stock assessment and targeted data collection because the attributes remain available for inspection before archetype assignment.
Administrative records and reconstructed geometry should remain the preferred sources where they are available, while \ac{SVI} can supplement missing visible or semantic attributes.
When the registered energy class is the only required output, direct image prediction provided stronger class balanced performance than prediction based on the evaluated structured attributes.
However, neither procedure should replace an \ac{EPC}, a physical survey, or measured performance assessment.
The appropriate use of \ac{SVI} therefore depends on whether the application requires explicit building attributes, archetype parameters, or an energy class prediction.
...
The comparison evaluates a fully fine tuned ResNet-50, a frozen DINOv2 backbone with supervised prediction layers, and InternVL3-2B without parameter updating using the same holdout buildings and attribute definitions.
Evaluating all three configurations on the same buildings distinguishes their performance for construction year, building type, and floor count and identifies the strengths and limitations of each configuration.

\textbf{The second contribution connects attribute errors to archetype assignment and thermal parameters.}
The building level dataset links Mapillary imagery, administrative reference attributes, reconstructed 3DBAG geometry, \ac{EPC} records, and TABULA-NL archetypes while preserving the data source of each value.
...
Reference attributes and \ac{SVI}-derived attributes are compared on the same buildings and data partitions using the same fitted classifier.
